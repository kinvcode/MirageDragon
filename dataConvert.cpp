...
String DataConvert::Utf16ToUtf8(const WString& wstr)
{
	return Utf16ToUtf8(wstr.c_str());
}

String DataConvert::Utf16ToUtf8(const wchar_t* wstr)
{
...
WString DataConvert::Utf8ToUtf16(const String& str)
{
	return Utf8ToUtf16(str.c_str());
}

WString DataConvert::Utf8ToUtf16(const char* str)
{
...
String DataConvert::LocalCpToUtf8(const CString& str)
{
	String convertedString = CW2A(str);
	return LocalCpToUtf8(convertedString.c_str());
}

String DataConvert::LocalCpToUtf8(const String& str)
{
	return LocalCpToUtf8(str.c_str());
}

String DataConvert::LocalCpToUtf8(const char* str)
{
	return Utf16ToUtf8(LocalCpToUtf16(str).c_str());
}

WString DataConvert::LocalCpToUtf16(const String& str)
{
	return LocalCpToUtf16(str.c_str());
}

WString DataConvert::LocalCpToUtf16(const char* str)
{
	WString convertedString;
	if (!str || !*str)
		return convertedString;
...
String DataConvert::Utf16ToLocalCp(const WString& str)
{
	String convertedString;
	if (str.size() == 0)
		return convertedString;
...
//json DataConvert::toJsonRoleList(vector<ROLEINFO> list) 
//{
//	json json_array;
//
//	return json_array;
